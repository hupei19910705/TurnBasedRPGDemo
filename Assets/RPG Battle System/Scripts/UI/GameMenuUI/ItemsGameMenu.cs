...
public class ItemsGameMenu : MonoBehaviour {
    ...
    void PopulateList () {
		...
		foreach (var item in MainSimple.ItemList) {
    ...
    public void ToggleSelectAction(Toggle toggle)
	{
        ...
        if (toggle.isOn) {
			...
			var itemDatas=MainSimple.ItemList.Where(w =>w.Name == toggleItem.Name.text).FirstOrDefault();
    ...
    public void ToggleUseAction(Toggle toggle)
	{
        ...
        if (toggle.isOn) {
			ItemsUI toggleItem = selectedToggle.GetComponent <ItemsUI> ();
			var x = MainSimple.ItemList.Where (w => w.Name == toggleItem.Name.text).FirstOrDefault ();
			...
			MainSimple.ItemList.Remove(MainSimple.ItemList.Where(w =>w.Name == toggleItem.Name.text ).FirstOrDefault());
