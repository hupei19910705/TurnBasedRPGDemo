...
public class EquipementsGameMenu : MonoBehaviour {
    ...
    void PopulateList () {
		...
		foreach (var item in MainSimple.EquipmentList) {
    ...
    public void ToggleSelectAction(Toggle toggle)
	{
        ...
        if (toggle.isOn) {
			...
			var itemDatas=MainSimple.EquipmentList.Where(w =>w.Name == toggleItem.Name.text).FirstOrDefault();
        ...
        if (toggle.isOn) {
			...
			case EnumEquipmentType.Head : 
				if (GameMenu.SelectedCharacter.Head != default(ItemsData))
                    {
                        MainSimple.EquipmentList.Where(w => w.Name == GameMenu.SelectedCharacter.Head.Name).FirstOrDefault().IsEquiped = false;
                    }
			...
			case EnumEquipmentType.LeftHand :
				...
				{
					MainSimple.EquipmentList.Where(w =>w.Name == GameMenu.SelectedCharacter.RightHand.Name).FirstOrDefault().IsEquiped = false;
					GameMenu.SelectedCharacter.RightHand = default(ItemsData);
				}

				if (GameMenu.SelectedCharacter.LeftHand != default(ItemsData))
                    {
                        MainSimple.EquipmentList.Where(w => w.Name == GameMenu.SelectedCharacter.LeftHand.Name).FirstOrDefault().IsEquiped = false;
                    }
    ...
    public void ToggleRemoveAction(Toggle toggle)
	{
        ...
        if (toggle.isOn) {
			...
			MainSimple.EquipmentList.Remove(MainSimple.EquipmentList.Where(w =>w.Name == toggleItem.Name.text && w.IsEquiped ==false).FirstOrDefault());
			ClearItemList();
			PopulateList ();
		}
	}
    ...
    public void ToggleRemoveAllAction(Toggle toggle)
	{
        ...
        if (toggle.isOn) {
			...
			MainSimple.EquipmentList.RemoveAll(r=>r.IsEquiped ==false);
			ClearItemList();
			PopulateList ();
		}
	}
