...
public class GameMenu : MonoBehaviour
{
    ...
    void Awake()
    {
     GetComponent<Canvas>().enabled = false;
		...
		SelectedCharacter = MainSimple.CharacterList.FirstOrDefault ();
    ...
    public void ShowHideMenu()
    {
        if (!IsInputLocked())
        {
            ...
            LockInput();
            if (MainSimple.IsGamePaused) {  MainSimple.PauseGame(false); }
            else
            {
                DisplayPanel(EnumWorldMenudAction.Team);
                if (TeamToggle) TeamToggle.isOn = true;
                MainSimple.PauseGame(true); }
    ...
    public void ToggleSaveAction(GameObject gameObject)
    {
        SoundManager.UISound();
        var toggle = gameObject.GetComponent<Toggle>();
        MainSimple.CurrentScene = SceneManager.GetActiveScene().name;
		if (toggle.isOn)
        { MainSimple.Save ();
